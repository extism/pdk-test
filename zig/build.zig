...
pub fn build(b: *std.Build) !void {
    ...
    const plugin_example_step = b.step("kitchen_sink", "Build plugin");
    plugin_example_step.dependOn(b.getInstallStep());

    // Run test using extism CLI
    const args = [_][]const u8{ "extism", "call", plugin.out_filename, "kitchen_sink", "--allow-host", "extism.org", "--input", "Hello!" };
